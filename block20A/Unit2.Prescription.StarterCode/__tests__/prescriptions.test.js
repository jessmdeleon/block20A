...
describe("applyDiscount()", () =>{
  describe("handles expected input", () =>{
//similar code from getTotalCost() ensuring the discount is applied based on the subscription status
    it("returns the correct cost afer applying sunscription disocunt", () => {
      expect(applyDiscount(100, true)).toBe(75);
    });
    });

describe.skip("handless unexpected input", () => {
//similar code from getTotalCost() check the function behaves with unexpected input 
  it("returns the original cost if total cost is not a number", () => {
    expect(applyDiscount("not a number", true)).toBe(NaN);
  });
  }); 
  });
...
describe("applyCoupon()", () =>{
  describe("handles expected input", () => {
//similar code from getTotalCost() ensure the coupon discount is applied correctly based on coupon status 
    it("returns the correct cost after applying coupon discount", () =>{
      expect(applyCoupon(100, true)).toBe(90);
    });
  }); 

describe.skip("handles unexpected input", () => {
  //similar code from getTotalCost() check how the function behaves with unexpected input
  it("returns the original cost if cost after subscription is not a number", () =>{
    expect(applyCoupon("not a number", true)).toBe(NaN);
    });
  });
});
